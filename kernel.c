...
// These symbols are defined in your linker script
...
struct process *create_process(uint32_t pc) {
  ...
  if (pc == 0 && (proc - procs) != 0)
    PANIC("create_process called with NULL pc!");
...
void yield(void) {
  ...
  // Update sscratch to next process's kernel stack top
